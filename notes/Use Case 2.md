---
tags: argo-write
title: 'Use Case 2: Forward Documents'
---

### Use Case 2: Forward Documents


<!-- Enter your content here -->
In this simple example, observations from an outside Health system uploaded by the patient's app are forwarded to the provider. US Core Providence is included to record the data provenance.

#### Description

Patient X has historical observations from Provider Y uploaded on his patient app. Provider Z is interested in reviewing this data and instructs patient to download the data to the patients record via the app.
...
- The server accepts all the data for review based on established policy (Argo Write limits in place as needed)
...
2. The App creates a FHIR Observation Example resource with a patient-supplied tag and submission key and device data and extension indicating the data was directly read from the device vs being hand-entered:
    - See below
...
:thinking_face: Would it be more consistent to use `meta.tag` (or `meta.security`) approach as we are proposing for 'patient-supplied' tagging?
